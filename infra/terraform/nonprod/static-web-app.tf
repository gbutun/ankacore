...
resource "azurerm_resource_group" "ank_app" {
  name     = local.resource_group_name
  location = var.location
  tags     = local.merged_tags
}

resource "azurerm_static_web_app" "ank_app" {
  name                = local.static_web_app_name
  resource_group_name = azurerm_resource_group.ank_app.name
  location            = azurerm_resource_group.ank_app.location
  sku_tier            = var.static_web_app_sku_tier
  sku_size            = var.static_web_app_sku_size
  tags                = local.merged_tags
}

resource "azapi_resource" "ank_app_custom_domain" {
  ...
  parent_id = azurerm_static_web_app.ank_app.id
